...
int MOVE_S(int idx)
{
    ...
    for (int i = 0; i < M_list.size(); i++) {
        ...
        else if (S.x + dir_s[S.d].first == M_list[i].x && S.y + dir_s[S.d].second == M_list[i].y) {
            if (CHECK_H(M_list[i].x, M_list[i].y)) continue;
            sum++;
        }
        else if (S.x + dir_s[S.d].first * 2 == M_list[i].x && S.y + dir_s[S.d].second * 2 == M_list[i].y) {
            if (CHECK_H(M_list[i].x, M_list[i].y)) continue;
            sum++;
        }
    }
...
{
    for (int i = 0; i < M_list.size(); i++) {
        cout << M_list[i].x << ' ' << M_list[i].y << ' ' << M_list[i].d << M_list[i].dd << endl;
    }
}
